...
class GeminiQueryAdapter extends QueryAdapter {
    constructor() {
        super();
        this.ai = new GoogleGenAI({ apiKey: process.env[GEMINI_CONFIGURATION.GEMINI_API_KEY_ENV_VAR]});
    }
    ...
    async createEmbedding(input) {
        ...
        try {
            ...
            if (!result.embeddings || result.embeddings.length === 0) {
                this.log('ERROR', 'Invalid embedding response - no embeddings', { result });
                throw new Error('Invalid embedding response from Gemini API: no embeddings returned');
            }

            const embedding = result.embeddings[0]?.values;
            if (!Array.isArray(embedding)) {
                this.log('ERROR', 'Embedding is not an array of numbers', { embedding });
