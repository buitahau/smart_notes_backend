...
export const authenticateToken: MiddlewareHandler<{ Variables: Variables }> = async (c, next) => {
    try {
        ...
        const cachedUserId = cacheService.get<string>(`iamId:${iamId}`);

        let userRecord: User | null = null;

        if (cachedUserId) {
            // Try getting cached user data
            userRecord = cacheService.get<User>(`user:${cachedUserId}`);
