...
const validateAndNormalize = (content: string, date: string, category: string): { ok: boolean, errorMessage?: string, normalizedData?: Note } => {
  ...
  return { ok: true, normalizedData: Note.from(normalizeContent, normalizeCategory, normalizeDate) };
};
...
class NoteController {
  async createNote(c) {
    try {
      ...
      const result = await noteService.createNote(normalizedData);
  ...
  async updateNote(c) {
    try {
      ...
      normalizedData.userId = userId;
      const result = await noteService.updateNote(normalizedData);
